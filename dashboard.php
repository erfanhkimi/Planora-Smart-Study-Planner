...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_course'])) {
        ...
        $stmt = $pdo->prepare("DELETE FROM User_Course WHERE UserID = ? AND CourseID = ?");
        $stmt->execute([$user_id, $course_id]);
        header("Location: dashboard.php");
        exit();
    }
    ...
    if (isset($_POST['mark_schedule_done'])) {
        ...
        $stmt = $pdo->prepare("UPDATE Schedule SET ScheduleStatus = 'completed' WHERE ScheduleID = ? AND UserID = ?");
    ...
    if (isset($_POST['delete_schedule'])) {
        $schedule_id = $_POST['schedule_id'];
        $stmt = $pdo->prepare("DELETE FROM Schedule WHERE ScheduleID = ? AND UserID = ?");
        $stmt->execute([$schedule_id, $user_id]);
        header("Location: dashboard.php");
        exit();
    }
    ...
    if (isset($_POST['reset_progress'])) {
        $stmt = $pdo->prepare("DELETE FROM Schedule WHERE UserID = ? AND (ScheduleStatus = 'completed' OR startDateTime < NOW())");
        $stmt->execute([$user_id]);
        header("Location: dashboard.php");
        exit();
    }
    ...
    if (isset($_POST['generate_ai_multiday'])) {
        ...
        // GROQ API
        $apiKey = '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';
        $url = 'https://api.groq.com/openai/v1/chat/completions';

        for ($i = 0; $i < $days; $i++) {
            $study_date = date('Y-m-d', strtotime("+$i day"));

            // Check for existing busy times
            $stmt = $pdo->prepare("
            ...
            $endDateTime = date('Y-m-d H:i:s', strtotime("$startDateTime +$hours_per_day hours"));
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?, ?, 0)
            ");
            $stmt->execute([$user_id, $course_id, $startDateTime, $endDateTime, $deadline]);
        }
...
if (isset($_POST['update_task_status'])) {
    ...
    $stmt = $pdo->prepare("UPDATE Task SET TaskStatus = 'completed' WHERE TaskID = ? AND UserID = ?");
...
if (isset($_POST['delete_task'])) {
    $task_id = $_POST['task_id'];
    $stmt = $pdo->prepare("DELETE FROM Task WHERE TaskID = ? AND UserID = ?");
    $stmt->execute([$task_id, $user_id]);
    header("Location: dashboard.php");
    exit();
}
...
<body class="bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 min-h-screen">
                            ...
                            <?php foreach ($user_courses as $idx => $course): ?>
                                <div class="relative rounded-xl p-4 text-center" style="background-color: <?php echo getColorForCourse($idx); ?>;">
                                    ...
                                    <form method="POST" class="absolute top-1 right-1">
                                        ...
                                        <button type="submit" name="delete_course" onclick="return confirm('Remove this course?')" class="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full hover:bg-red-600">
                                            ✕
                                        </button>
                                    </form>
                                    ...
                                    <div class="flex-1 rounded-2xl px-5 py-3" style="background-color: <?php echo getColorForCourse($idx); ?>;">
                                        ...
                                        <div class="flex items-center gap-2 mt-2">
                                            ...
                                            <form method="POST" onsubmit="return confirm('Delete this schedule?')">
                                                ...
                                                <button type="submit" name="delete_schedule" class="text-red-600 hover:text-red-800 font-semibold">
                                                    ✕
                                                </button>
                                            </form>
                                        </div>
                        ...
                        <div class="space-y-3">
                            <?php foreach ($all_schedules as $idx => $schedule): ?>
                                <div class="flex items-center gap-4">
                                    ...
                                    <form method="POST" class="inline">
                                        <input type="hidden" name="schedule_id" value="<?= $schedule['ScheduleID']; ?>">
                                        <button type="submit" name="mark_schedule_done" class="text-xs bg-green-500 text-white px-3 py-1 rounded-full hover:bg-green-600">
                                            ✔
                                        </button>
                                    </form>
                                    <!-- DELETE BUTTON -->
                                        ...
                                        <div class="ml-8">
                                            <div class="flex items-center gap-2 text-sm">
                                                ...
                                                <div class="flex-1 bg-gray-200 rounded-full h-2">
                                                    <div class="bg-gradient-to-r from-teal-400 to-cyan-400 h-2 rounded-full" style="width: <?php echo $task['completionRates']; ?>%"></div>
                                                </div>
                                                <form method="POST" class="inline-flex items-center gap-1">
                                                    <input type="hidden" name="tracker_id" value="<?php echo $task['TrackerID']; ?>">
                                                    <input type="number" name="completion_rate" min="0" max="100" value="<?php echo $task['completionRates']; ?>" class="w-12 px-1 py-0.5 border rounded text-xs">
                                                    <button type="submit" name="update_completion" class="text-xs bg-purple-500 text-white px-2 py-0.5 rounded">✓</button>
                                                </form>
                                            </div>
                                        </div>
                                    <?php endif; ?>
                ...
                <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-3xl shadow-lg p-6 border border-purple-200">
                    ...
                    <form method="POST" class="space-y-3">
                        ...
                        <div>
                            ...
                            <select name="ai_course_selection" required class="w-full px-3 py-2 border border-purple-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
                        ...
                        <!-- Prompt Text Area -->
                        <textarea name="prompt_text" rows="3" placeholder="Ask for study tips, techniques, or strategies..." required class="w-full px-3 py-2 border border-purple-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"></textarea>
                        
                        <button type="submit" name="ask_ai" class="w-full bg-purple-500 hover:bg-purple-600 text-white px-4 py-2.5 rounded-xl text-sm transition-colors font-semibold">
                ...
                <form method="POST">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="course_name" required placeholder="e.g., Mathematics, Physics, Programming" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-3xl p-8 max-w-md w-full mx-4">
                ...
                <form method="POST" class="space-y-4">
                    <div>
                        ...
                        <select name="ai_course_id" required class="w-full px-4 py-2 border rounded-lg">
                            <?php foreach ($user_courses as $course): ?>
                    ...
                    <div>
                        ...
                        <input type="number" name="total_hours" min="1" max="40" required class="w-full px-4 py-2 border rounded-lg">
                    </div>
                    <div>
                        <label class="text-sm font-medium">Number of Days</label>
                        <input type="number" name="days" min="1" max="14" required class="w-full px-4 py-2 border rounded-lg">
                    </div>
                    <div>
                        <label class="text-sm font-medium">Preferred Time</label>
                        <select name="preferred_time" class="w-full px-4 py-2 border rounded-lg">
                    ...
                    <div class="flex gap-3 pt-4">
                        <button type="submit" name="generate_ai_multiday" class="flex-1 bg-purple-500 text-white py-2 rounded-lg">
                            Generate Plan
                        </button>
                        <button type="button" onclick="closeModal('aiMultiDayModal')" class="flex-1 bg-gray-200 py-2 rounded-lg">
                            Cancel
                        </button>
                    </div>
                </form>
                ...
                <form method="POST">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" required placeholder="e.g., Complete homework, Study chapter 5" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                            <textarea name="description" rows="3" placeholder="Add more details about this task..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"></textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="schedule_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                        ...
                        <div>
                            ...
                            <input type="date" name="deadline" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                        </div>
                    </div>
                    <div class="flex gap-3 mt-6">
                        <button type="submit" name="add_task" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg transition-colors">
                            Add Task
                        </button>
                        <button type="button" onclick="closeModal('taskModal')" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg transition-colors">
                            Cancel
                        </button>
                    </div>
                </form>
    ...
    <?php if ($total > 0 && $weekly_progress >= 100): ?>
        <div id="congratsModal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[100] animate-fade-in" style="display: flex;">
            <div class="bg-white p-8 rounded-3xl shadow-2xl text-center max-w-sm mx-4 transform transition-all scale-110 border-4 border-yellow-400">
                ...
                <form method="POST">
                    <button type="submit" name="reset_progress" class="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold py-4 rounded-xl shadow-lg transform active:scale-95 transition-all">
                        Clear History & Start Fresh
                    </button>
                </form>
                
                <button onclick="document.getElementById('congratsModal').style.display='none'" class="mt-4 text-sm text-gray-400 hover:text-gray-600 underline">
